...
import type { OutfitPlan } from '@/types/project-plan';
import { generateContent as generateWithNano } from '@/lib/gemini-nano';
import { generateWithApi } from '@/lib/gemini-api';
import { exportProjectToPPT, exportToPPT, ShootingPlan } from '@/lib/ppt-exporter';
...
import { Sparkles, FileDown, Loader2, Info, Image as ImageIcon, Wand2 } from 'lucide-react';
...
export default function Home() {
  ...
  const generateImage = async (visualPrompt: string, index: number) => {
    ...
    try {
      ...
      if (!response.ok) throw new Error(data.error || `HTTP 错误: ${response.status}`);
  ...
  const handleGenerateProject = async () => {
    ...
    setLoading(true);
    try {
      const prompt = buildProjectPrompt({ client: clientProfile as ClientProfile, outfits: cleanedOutfits });

      const mode = localStorage.getItem('gemini_mode') || 'nano';
      const apiKey = localStorage.getItem('gemini_api_key') || '';

      let response;
      if (mode === 'api' && apiKey) {
        response = await generateWithApi(apiKey, prompt);
      } else {
                    ...
                    <div className="grid gap-4 md:grid-cols-4">
                      ...
                      <div className="rounded-[var(--radius-xl)] border border-[var(--line)] bg-white p-4 shadow-sm">
                        ...
                        <div className="mt-2 flex flex-col gap-2">
                          <button
                            type="button"
                            onClick={() => exportProjectToPPT(projectPlan, { images: sceneImages })}
                              ...
                              <div className="p-6 space-y-3">
                                ...
                                {s.type === '纯色版面' && s.cutoutSpec && (
                                  <div className="rounded-xl border border-[var(--line)] bg-white p-4 text-sm text-[var(--ink)]">
                                    <div className="text-[10px] font-semibold text-[var(--ink-3)] tracking-widest mb-2">纯色版面（抠图/排版）</div>
                ...
                <div className="space-y-10">
                  ...
                  <div className="flex flex-col md:flex-row md:items-end md:justify-between gap-4">
                    ...
                    <div className="flex items-center gap-3">
                      ...
                      <button
                        onClick={() => exportToPPT(plan, { images: sceneImages })}
                        className="inline-flex items-center justify-center gap-2 rounded-xl bg-[var(--accent)] px-5 py-2.5 text-sm font-semibold text-white shadow-sm hover:opacity-95 transition"
                      >
                  ...
                  <div className="grid gap-4 md:grid-cols-4">
                    ...
                    <div className="rounded-[var(--radius-xl)] border border-[var(--line)] bg-white p-4 shadow-sm">
                      <div className="text-[10px] font-semibold text-[var(--ink-3)] tracking-widest">导出</div>
                      <div className="mt-2 text-2xl font-semibold text-[var(--ink)]">PPT</div>
                    </div>
                  </div>
                </div>
